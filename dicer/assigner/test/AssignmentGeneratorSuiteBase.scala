...
abstract class AssignmentGeneratorSuiteBase(
  ...
  test("Assignment synced from subscriber") {
    ...
    val (driver, store): (AssignmentGeneratorDriver, InterceptableStore) =
      createDriverAndStore(assignerServiceInfoOpt = None, storeIncarnationOpt = Some(incarnation))
  ...
  test(
    ...
    val (driver, store): (AssignmentGeneratorDriver, Store) =
      createDriverAndStore(assignerServiceInfoOpt = None, targetConfig)
    val rng = new Random()
  ...
  test("Stale assignment from in-memory store doesn't pollute initial assignment generation") {
    ...
    val driver: AssignmentGeneratorDriver =
      createDriverUsingStore(store, assignerServiceInfoOpt = None)
  ...
  protected def createDriverAndStore(
    ...
    (
      createDriverUsingStore(
        ...
        dicerSimulatorEventLogEmitter = dicerSimulatorEventLogEmitter,
  ...
  private def createSliceletRequest(
    ...
    createSliceletRequestForSquid(resource(id), syncState, state, attributedLoads, target)
  }

  /**
   * Creates a client request from the Slicelet identified by `squid`, with the given `syncState`,
   * `attributedLoads`, and reported `state`. Unlike the `id`-based overload, this lets a caller
   * supply an explicit SQUID -- e.g. one with an advanced creation time to model a new incarnation
   * of the same resource.
   */
  private def createSliceletRequestForSquid(
      squid: Squid,
      syncState: SyncAssignmentState,
  ...
  test("No assignment within initialization delay") {
    ...
    val targetConfig: InternalTargetConfig = defaultTargetConfig
    val driver: AssignmentGeneratorDriver =
      createDriver(assignerServiceInfoOpt = None, targetConfig)
  ...
  test("Test initial assignment") {
    ...
    // an assignment is generated for the given resource and contains the expected assigner
    // service info.
    val expectedAssignerServiceInfo: AssignerServiceInfo =
      AssignerServiceInfo(name = "test-assigner", instanceId = "test-instance")
    val targetConfig: InternalTargetConfig = defaultTargetConfig
    val driver: AssignmentGeneratorDriver = createDriver(
      Some(expectedAssignerServiceInfo),
      targetConfig
    )
    ...
    assert(assignment.assignerServiceInfoOpt == Some(expectedAssignerServiceInfo))
  }
  ...
  namedGridTest("Test updated assignment with changing resources")(
    ...
    // `healthCheckPeriod`. Verify that all generated assignments (new and updated) contain the
    // expected assigner service info.
    val targetConfig: InternalTargetConfig = defaultTargetConfig
  ...
  test("Available resources change while write in progress") {
    ...
    val (driver, store): (AssignmentGeneratorDriver, InterceptableStore) =
      createDriverAndStore(assignerServiceInfoOpt = None)
  ...
  test("Retry on write failure") {
    ...
    val generator: AssignmentGenerator =
      createGenerator(createGeneratorConfig(), targetConfig, clock, assignerServiceInfoOpt = None)
  ...
  test("Assignment stats published to metrics only on successful write") {
    ...
    val generator: AssignmentGenerator =
      createGenerator(generatorConfig, targetConfig, clock, assignerServiceInfoOpt = None)
  ...
  test("No assignment generation for frozen assignment") {
    ...
    val unfrozenWriteResult = TestUtils.awaitResult(
      store.writeAssignment(
        ...
        shouldFreeze = false,
        ProposedAssignment(
          predecessorOpt = Some(frozenAssignment),
          sliceMap = proposal,
          assignerServiceInfoOpt = None
        )
      ),
  ...
  test("Watch from Slicelet with empty k8s namespace does not trigger k8s watch, fires alert") {
    ...
    val generator: AssignmentGenerator =
      createGenerator(config, targetConfig, clock, assignerServiceInfoOpt = None)
  ...
  test("Watches from remote cluster Slicelets does not trigger empty/mismatched namespace alerts") {
    ...
    val config: AssignmentGenerator.Config = createGeneratorConfig()
    val generator: AssignmentGenerator =
      createGenerator(config, defaultTargetConfig, clock, assignerServiceInfoOpt = None, target)
  ...
  test("Assignment generation triggered at regular intervals when LB is enabled") {
    ...
    val generatorConfig: AssignmentGenerator.Config = createGeneratorConfig()
    val generator: AssignmentGenerator =
      createGenerator(generatorConfig, targetConfig, fakeClock, assignerServiceInfoOpt = None)
  ...
  test("Assignment generator stops generating assignments after shutdown") {
    ...
    val driver: AssignmentGeneratorDriver = createDriver(assignerServiceInfoOpt = None)
  ...
  test("Terminating Slicelet rejoins assignment after a new resource incarnation reports Running") {
    // Test plan: verify that a Slicelet which reports TERMINATING may rejoin the assignment after
    // a new resource incarnation with the same resource UUID reports RUNNING.
    val driver: AssignmentGeneratorDriver = createDriver(assignerServiceInfoOpt = None)

    // resource 7 and the anchor resource 8 report Running; advance past the initial health report
    // delay so the first assignment is generated with both.
    sec.advanceBySync(20.seconds)
    driver.onWatchRequest(createSliceletRequest(7, Generation.EMPTY))
    driver.onWatchRequest(createSliceletRequest(8, Generation.EMPTY))
    sec.advanceBySync(10.seconds)
    val assignment1: Assignment = awaitNewerAssignment(driver, Generation.EMPTY)
    assert(assignment1.assignedResources == Set(resource(7), resource(8)))

    // resource 7 reports TERMINATING and is dropped from the assignment; the anchor remains.
    driver.onWatchRequest(
      createSliceletRequest(
        id = 7,
        syncState = SyncAssignmentState.KnownGeneration(assignment1.generation),
        state = SliceletDataP.State.TERMINATING
      )
    )
    val assignment2: Assignment = awaitNewerAssignment(driver, assignment1.generation)
    assert(assignment2.assignedResources == Set(resource(8)))

    // A newer incarnation of resource 7 (same resource UUID, later creation time) reports RUNNING
    // and rejoins the assignment under its new SQUID, since the prior incarnation's terminating
    // status no longer applies.
    val newerCreationTime: Instant = resource(7).creationTime.plusSeconds(1000)
    val newPod7: Squid = resource(7).copy(creationTimeMillis = newerCreationTime.toEpochMilli)
    driver.onWatchRequest(
      createSliceletRequestForSquid(
        newPod7,
    ...
    val assignment3: Assignment = awaitNewerAssignment(driver, assignment2.generation)
    assert(assignment3.assignedResources == Set(newPod7, resource(8)))
  }

  test("Generator shutdown clears gauge metrics") {
    // Test plan: verify that generator shutdown clears gauge metrics to avoid confusion caused by
    // stale values.
    val targetConfig: InternalTargetConfig = defaultTargetConfig
    ...
    val (driver1, store1): (AssignmentGeneratorDriver, InterceptableStore) =
      createDriverAndStore(assignerServiceInfoOpt = None, targetConfig, target = target1)
    // driver2 and driver1 should have the same store incarnation.
    val driver2: AssignmentGeneratorDriver =
      createDriver(
        assignerServiceInfoOpt = None,
        targetConfig,
        Some(store1.storeIncarnation),
        target = target2
      )
    ...
    // resource 4 stays Running throughout so the Running podset size remains non-zero after
    // resources 0 and 1 terminate below.
    val request1c = createSliceletRequestForTarget(4, Generation.EMPTY, target = target1)
    driver1.onWatchRequest(request1a)
    driver1.onWatchRequest(request1b)
    driver1.onWatchRequest(request1c)
    ...
    // Terminate resource 0 via Kubernetes pod termination (-> ResourceTerminating) and resource 1
    // via a Slicelet TERMINATING report (-> ResourceIncarnationTerminating). resource 4 remains
    // Running.
    sec.advanceBySync(1.second)
    fakeKubernetesTargetWatcherFactory.simulateTerminationSignal(
    ...
    AssertionWaiter("gauge metrics are updated for target1").await {
      ...
      assert(TargetMetricsUtils.getPodSetSize(target1, "ResourceIncarnationTerminating") > 0)
      assert(TargetMetricsUtils.getPodSetSize(target1, "ResourceTerminating") > 0)
  ...
  test("Known prior incarnation assignment from slicelet bypasses initialization delay") {
    ...
    val (driver, _): (AssignmentGeneratorDriver, Store) =
      createDriverAndStore(assignerServiceInfoOpt = None, targetConfig, Some(currentIncarnation))
    val rng = new Random()
  ...
  test("Higher incarnation assignment is distributed") {
    ...
    val driver: AssignmentGeneratorDriver =
      createDriver(
        assignerServiceInfoOpt = None,
        targetConfig,
        storeIncarnationOpt = Some(incarnation)
      )
    val higherIncarnation: Incarnation = createLargerValidIncarnation(incarnation)
  ...
  test("State transfer information is generated") {
    ...
    val driver: AssignmentGeneratorDriver =
      createDriver(assignerServiceInfoOpt = None, defaultTargetConfig)
  ...
  gridTest("Bootstraps from assignment in previous store incarnation")(
    ...
    val (driver, _): (AssignmentGeneratorDriver, InterceptableStore) =
      createDriverAndStore(assignerServiceInfoOpt = None, targetConfig, Some(incarnation))
  ...
  test("Bootstrapping incorporates new load signals") {
    ...
    val driver: AssignmentGeneratorDriver =
      createDriver(
        assignerServiceInfoOpt = None,
        targetConfig,
        Some(createLargerValidIncarnation(initialIncarnation))
      )
  ...
  test("key cardinality estimate") {
    // Test plan: verify that key cardinality estimates delivered over Watch end up in the target
    // metric.
    ...
    // Send a few Watch requests with overlapping keys in the cardinality estimate and make sure
    // they end up merged to match the HLL that we put all of the keys into.
    val oracle = new HyperLogLog()
    for (i <- 0 until 3) {
      val requestHll = new HyperLogLog()
      val start = 500 * i
      val end = start + 1000
      for (j <- start until end) {
        val key = ByteString.copyFrom(BigInt(j.toLong).toByteArray)
        requestHll.add(key)
        oracle.add(key)
      }
      ...
      val sliceletRequest = ClientRequest(
        ...
        SliceletData(
          ...
          unattributedLoadOpt = None,
          keyCardinalityEstimateOpt = Some(requestHll)
        ),
        supportsSerializedAssignment = true,
        redirectTokenOpt = None,
        alternativeTargetOpt = None,
    ...
    val expected = oracle.estimate()
    AssertionWaiter("waiting for key cardinality estimate update").await {
      assert(TargetMetricsUtils.getKeyCardinalityEstimate(target).get == expected.toDouble)
    }

    // The key cardinality estimate is only for recently seen keys, so if we wait long enough the
    // estimate without reporting any it should return to zero.
    clock.advanceBy(600.seconds)
    generator.forTest.onAdvance(clock.tickerTime(), clock.instant())

    AssertionWaiter("waiting for key cardinality estimate update").await {
      assert(TargetMetricsUtils.getKeyCardinalityEstimate(target).get == 0)
    }
  }
}

object AssignmentGeneratorSuiteBase {

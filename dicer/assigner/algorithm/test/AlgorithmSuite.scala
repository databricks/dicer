...
class ParameterizedAlgorithmSuite(keyReplicationConfig: KeyReplicationConfig)
  ...
  /**
   * Verifies that the homomorphic assignment generated satisfies the following properties:
   * - Slice boundaries of the homomorphic assignment must be the same as the predecessor
   *   assignment, as well as the corresponding slice load information and replication number.
   * - Resources that appear in both assignments must have the same set of slices assigned.
   */
  private def verifyHomomorphicAssignment(
      predecessor: Assignment,
      homomorphicAssignment: Assignment
  ) {
    val predecessorEntries: Vector[SliceAssignment] = predecessor.sliceMap.entries.toVector
    val homomorphicAssignmentEntries: Vector[SliceAssignment] =
      homomorphicAssignment.sliceMap.entries.toVector

    // Verify that slice boundaries, per-slice load information, and number of replicas per slice
    // are the same.
    assert(homomorphicAssignmentEntries.size == predecessorEntries.size)
    for (tuple <- homomorphicAssignmentEntries.zip(predecessorEntries)) {
      val (homomorphicSliceAssignment, predecessorSliceAssignment): (
          SliceAssignment,
          SliceAssignment) = tuple
      assert(homomorphicSliceAssignment.slice == predecessorSliceAssignment.slice)
      assert(
        homomorphicSliceAssignment.primaryRateLoadOpt ==
        predecessorSliceAssignment.primaryRateLoadOpt
      )
    }

    // Compute the resource to slices mappings for both assignments.
    val predecessorResourceToSlices: mutable.Map[Squid, Set[Slice]] =
    ...
    // Verify that resources that appear in both assignments have the same set of slices assigned.
    for (predecessorResource: Squid <- predecessorResourceToSlices.keys) {
      if (homomorphicResourceToSlices.contains(predecessorResource)) {
        val predecessorSlices: Set[Slice] = predecessorResourceToSlices(predecessorResource)
        val homomorphicSlices: Set[Slice] = homomorphicResourceToSlices(predecessorResource)
        assert(predecessorSlices == homomorphicSlices)
      }
    }
  }
  ...
  test("Homomorphic assignment where only unhealthy resources are introduced") {
    // Test plan: Verify that, given an initial assignment with a set of healthy resources,
    // the homomorphic assignment generated by the algorithm is the same as the initial assignment
    // when some resources become unhealthy while no new resources are added.
    val target = Target(getSafeName)
    val targetConfig: InternalTargetConfig = defaultTargetConfig

    // Generate an initial assignment with a set of healthy resources.
    val startingResources: Resources =
      createNResources(targetConfig.keyReplicationConfig.minReplicas)
    val initialAssignment: Assignment =
      generateInitialAssignment(target, startingResources, targetConfig.keyReplicationConfig)

    // Remove half of the healthy resources and generate a homomorphic assignment.
    val newResources: Resources = createNResources(startingResources.availableResources.size / 2)
    val newAssignment: Assignment =
    ...
    // boundaries and resource assignments.
    verifyHomomorphicAssignment(initialAssignment, newAssignment)
  }

  test("Homomorphic assignment where healthy resources replace unhealthy ones") {
    // Test plan: Verify that, given an initial assignment with a set of healthy resources and a
    // uniform load map, the homomorphic assignment generated by the algorithm should use new,
    // healthy resources to replace the unhealthy ones. In this test, all healthy resources in the
    // initial assignment are removed in the second, which contains a totally different set of
    // healthy resources.
    val target = Target(getSafeName)
    val targetConfig: InternalTargetConfig = defaultTargetConfig
    ...
    // assignment, providing an equal number of new, healthy resources.
    val newResources: Resources = createNResourcesFrom(
      startIndex = startingResources.availableResources.size,
      n = startingResources.availableResources.size
    )
    val newAssignment: Assignment =
      generateHomomorphicAssignment(target, newResources, initialAssignment)

    // Verify that the set of resources in the new assignment consists solely of the new, healthy
    // resources, though slice boundaries should be the same as in the initial assignment.
    assert(newAssignment.assignedResources == newResources.availableResources)
    verifyHomomorphicAssignment(initialAssignment, newAssignment)
  }
  ...
  test("Homomorphic assignment with extra healthy resources") {
    ...
    // by the algorithm should use some healthy resources to replace the unhealthy resources, while
    // the remaining healthy resources are not used.
    val target = Target(getSafeName)
    val targetConfig: InternalTargetConfig = defaultTargetConfig

    // Generate an initial assignment with a set of healthy resources.
    val startingResources: Resources =
      createNResources(2 * targetConfig.keyReplicationConfig.minReplicas)
    val initialAssignment: Assignment =
      generateInitialAssignment(target, startingResources, targetConfig.keyReplicationConfig)

    // Remove half of the healthy resources, keep the other half running, and add in another set of
    // healthy resources, for a total of 3 * minReplicas healthy resources.
    val newResources: Resources = createNResourcesFrom(
      startIndex = targetConfig.keyReplicationConfig.minReplicas,
      n = 3 * targetConfig.keyReplicationConfig.minReplicas
    )
    val newAssignment: Assignment =
      generateHomomorphicAssignment(target, newResources, initialAssignment)

    // Verify that the new assignment contains only 2 * minReplicas resources.
    assert(
      newAssignment.assignedResources.size == 2 * targetConfig.keyReplicationConfig.minReplicas
    )

    // Verify that the unhealthy resources have been removed, and that half of the new
    // resources have been used in the new assignment.
    val removedResources: Set[Squid] =
      createNResources(targetConfig.keyReplicationConfig.minReplicas).availableResources
    val newlyAddedResources: Set[Squid] = createNResourcesFrom(
      startIndex = 2 * targetConfig.keyReplicationConfig.minReplicas,
      n = 2 * targetConfig.keyReplicationConfig.minReplicas
    ).availableResources
    assert(newAssignment.assignedResources.intersect(removedResources).isEmpty)
    assert(
      newAssignment.assignedResources
        .intersect(newlyAddedResources)
        .size == targetConfig.keyReplicationConfig.minReplicas
    )

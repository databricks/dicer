 ...
 */
trait SubscriberHandlerHarness {
  ...
  /** Returns the current Slicez subscriber data tracked by the handler. */
  def getSlicezData(): (Seq[SliceletSubscriberSlicezData], Seq[ClerkSubscriberSlicezData])
